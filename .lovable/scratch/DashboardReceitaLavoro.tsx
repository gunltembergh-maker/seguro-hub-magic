...
import { Calendar, ChevronRight, ChevronDown, Clock, RefreshCw, TrendingUp, TrendingDown, Send, HelpCircle } from "lucide-react";
import { DashboardReceitaLavoroOnboardingModal } from "@/components/dashboards/DashboardReceitaLavoroOnboardingModal";
...
export default function DashboardReceitaLavoro() {
  ...
  useEffect(() => {
    if (!user?.id) return;
    const key = `dashboard_receita_lavoro_onboarding_${user.id}`;
    if (!localStorage.getItem(key)) setShowOnboarding(true);
  }, [user?.id]);
  const handleCloseOnboarding = () => {
    if (user?.id) localStorage.setItem(`dashboard_receita_lavoro_onboarding_${user.id}`, "1");
  ...
  return (
    <AppLayout>
      ...
      `}</style>
      <TailorFrame>
        <DashboardReceitaLavoroOnboardingModal open={showOnboarding} onClose={handleCloseOnboarding} />
