CREATE OR REPLACE FUNCTION public.normalize_canal_repasse(p_canal text)
RETURNS text LANGUAGE sql IMMUTABLE AS $$
  SELECT upper(translate(btrim(coalesce(p_canal,'')),
...
CREATE OR REPLACE FUNCTION public.rpc_lavoro_repasse_filtros()
RETURNS TABLE(tipo text, valor text)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $$
  SELECT 'canal_repasse'::text, canais.rotulo::text
  FROM (
    SELECT mode() WITHIN GROUP (ORDER BY btrim(g.possui_repasse)) AS rotulo
    FROM public.vw_lavoro_gerencial g
    WHERE public.normalize_canal_repasse(g.possui_repasse) NOT IN ('LAVORO','')
      AND coalesce(g.valor_repasse_total,0) <> 0
    GROUP BY public.normalize_canal_repasse(g.possui_repasse)
  ) canais
  UNION ALL
  SELECT 'situacao_repasse'::text, situacoes.s::text
  FROM (
    SELECT DISTINCT initcap(lower(btrim(coalesce(g.status_repasse,'')))) AS s
    FROM public.vw_lavoro_gerencial g
    WHERE lower(btrim(coalesce(g.status_repasse,''))) NOT IN ('','-')
  ) situacoes
  ORDER BY 1, 2;
$$;

GRANT EXECUTE ON FUNCTION public.normalize_canal_repasse(text) TO authenticated;
