...
SELECT
  ...
  COALESCE(dp.tipo_de_ramo, 'Sem Categoria') AS canal,
  g.seguradora,
  g.data_emissao,
  g.data_pagamento,
  ...
  g.ano::int AS ano,
  g.mes::int AS mes,
  COALESCE(g.comissao_bruta, 0)::numeric AS comissao_bruta,
  COALESCE(g.comissao_bruta, 0)::numeric AS comissao_liquida
FROM public.raw_lavoro_gerencial g
LEFT JOIN public.vw_lavoro_depara_ramo dp
  ON lower(btrim(dp.ramo)) = lower(btrim(g.ramo))
...
-- Recriar RPCs derrubados pelo CASCADE
...
BEGIN
  ...
  RETURN QUERY
    SELECT COALESCE(v.ramo,'(sem ramo)'), COALESCE(SUM(v.comissao_bruta),0)
...
BEGIN
  ...
  v_fim := CASE _periodo WHEN 'SEMESTRE' THEN CASE WHEN _mes<=6 THEN 6 ELSE 12 END ELSE _mes END;
  RETURN QUERY
    SELECT COALESCE(v.canal,'(sem canal)'), COALESCE(SUM(v.comissao_bruta),0)
      FROM public.vw_lavoro_receita_competencia v
     WHERE v.ano=_ano AND v.mes BETWEEN v_ini AND v_fim
     GROUP BY 1 ORDER BY 2 DESC;
END; $function$;
...
AS $function$
  SELECT ano, mes, COALESCE(SUM(comissao_bruta),0)
    FROM public.vw_lavoro_receita_competencia
   WHERE ano = ANY(_anos)
