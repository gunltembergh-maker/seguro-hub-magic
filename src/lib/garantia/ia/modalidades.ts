...
].map((m) => ({ ...m, produto: "seguro_garantia" as const }));
...
export const FL_MODALIDADES: ModalidadeWorker[] = [
  ...
  { id: "outra", label: "Outra modalidade" },
].map((m) => ({ ...m, produto: "fianca_locaticia" as const }));
...
export const modalidadeValida = (id: string | null | undefined): boolean => !!id && IDS_MODALIDADE.includes(id);
