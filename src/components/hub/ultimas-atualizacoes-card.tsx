...
export function UltimasAtualizacoesCard({ timestamps, isLoading }: { timestamps: TimestampRow[]; isLoading: boolean }) {
  return (
    <div className="rounded-lg bg-white/95 p-5 shadow-sm" style={{ border: "1px solid rgba(19,64,92,0.15)" }}>
      <h3 className="flex items-center gap-2 font-display text-base font-semibold text-[#13405C]">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        {!isLoading && timestamps.map((t) => (
          <div key={t.fonte} className="rounded-md border border-[#13405C]/10 bg-[#F8FAFC] p-3">
            ...
            <div className="mt-1 flex items-center justify-between text-sm">
              <span className="flex items-center gap-1.5 text-[#0E2E43]">
                <Clock className="h-3.5 w-3.5 text-[#338B85]" />
