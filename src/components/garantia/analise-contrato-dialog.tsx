...
export function AnaliseContratoDialog({ aberto, onFechar, demanda, documento, analise, onNovaAnalise }: Props) {
  ...
  const analisar = async (forcar = false) => {
    ...
    try {
      ...
      setPasso("Lendo o texto e identificando as páginas…");
      const arquivo = new File([blob], documento.nome_arquivo, { type: documento.mime_type ?? "application/pdf" });
      const extraido = await extrairConteudoArquivo(arquivo);
      setPasso("Analisando o contrato por partes…");
      const retorno = await executar({ data: { analiseId: alvo.id, arquivoExtraido: extraido } });
