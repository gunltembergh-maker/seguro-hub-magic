  ...
  return (
    <Dialog open={aberto} onOpenChange={(o) => !o && onFechar()}>
      <DialogContent className="max-h-[90dvh] w-[calc(100%-2rem)] max-w-6xl min-w-0 overflow-y-auto p-4 sm:p-6">
        <DialogHeader><DialogTitle>{local?.fluxo === "financeiro" ? "Análise financeira por IA" : "Análise do contrato por IA"}</DialogTitle><DialogDescription>{documento.nome_arquivo}{(local?.documentos_ids?.length ?? 0) > 1 ? ` e mais ${local!.documentos_ids.length - 1}` : ""}{local ? ` · solicitada em ${dataHora(local.criado_em)}` : ""}{solicitante ? ` por ${solicitante}` : ""}</DialogDescription></DialogHeader>
