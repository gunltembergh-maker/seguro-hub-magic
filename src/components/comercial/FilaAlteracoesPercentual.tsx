...
export function useAlteracoesPercentual(status?: string | null) {
  return useQuery({
    ...
    queryFn: async (): Promise<Alteracao[]> => {
      const { data, error } = await supabase.rpc("rpc_canal_parceiro_alteracoes" as never, {
...
export function FilaAlteracoesPercentual({ semCard = false }: { semCard?: boolean } = {}) {
  ...
  async function decidir(id: string, aprovar: boolean, obs: string | null) {
    setSalvando(true);
    try {
      const { error } = await supabase.rpc("rpc_canal_parceiro_decidir_alteracao" as never, {
        p_alteracao_id: id,
        p_aprovar: aprovar,
        p_observacao: obs,
      } as never);
        ...
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() =>
                recusando && decidir(recusando.alteracao_id, false, observacao.trim())
              }
            >
              {salvando && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Recusar
